...
- [x] Step 5: Refactor `src/components/PromoCarousel.tsx` to load slides from both `home_deals` and `promotions` ordered by priority; add fallback slide on empty/error
- [x] Step 6: Ensure carousel frosted text panel extends existing drape overlay (layering/z-index) and CTA always stands out
- [x] Step 7: Strengthen auth dropdown: sync with Supabase auth state (`onAuthStateChange`), ensure username fallback works, ARIA + outside click/escape
- [x] Step 8: Remove any standalone “Profile” link from navbar entirely (keep only dropdown)
